...
% showCellNums = cellsTuned;
% showCellNums = cellsNTResponse;
% showCellNums = tuneBroad;

showCellNums = intersect(tunedNF, tunedFitA);
...
%     cellNum = u.cellNums(cellid);
    ...
    spk = spkTotal{(cellsTotal == cellNum)};
    dfmean = cell2mat(cellfun(@(x) nanmean(x), spk, 'uniformoutput', false));
    %%
    % ANOVA
    touchNumGroups = cellfun(@(x) size(x,1), spk);
    timeAveragedF = zeros(sum(touchNumGroups),1);
    anovaGroupsF = zeros(sum(cellfun(@(x) size(x,1), spk)),1);
    for ai = 1 : length(angles)
        timeAveragedF( sum(touchNumGroups(1:(ai-1))) + 1 : sum(touchNumGroups(1:ai)) ) = mean(spk{ai}(:,baseFrameNum+1:end),2);
        anovaGroupsF( sum(touchNumGroups(1:(ai-1))) + 1 : sum(touchNumGroups(1:ai)) ) = deal(ai);        
    end
    
    %% ANOVA
    tTestH = cellfun(@(x) ttest(nanmean(x(:,baseFrameNum+1:baseFrameNum+afterFrameNum),2)), spk);
    tTestInd = find(tTestH);
    means = cellfun(@(x) mean(nanmean(x(:,baseFrameNum+1:baseFrameNum+afterFrameNum),2)), spk);
    sems = cell2mat(cellfun(@(x) std(nanmean(x(:,baseFrameNum+1:baseFrameNum+afterFrameNum),2))/sqrt(size(x,1)), spk, 'uniformoutput',false));
...
%     hold on
%     for i = 1 : size(spk,1)
%         message = ['n=',num2str(size(spk{i},1))];
%         text(1,i, message,'FontWeight', 'bold','FontSize',17,'Color','white');
%     end
    c = colorbar;
...
%     errorbar(angles(sigIndGroup), meanNsem(sigIndGroup,1), meanNsem(sigIndGroup,2), 'y.')
    ...
    set(gca,'XTick', angles, 'fontsize',17), xlabel('Pole angle (\circ)'), ylabel('\Delta spike #')
