...
for mi = 1 : length(mice)
    ...
    d = [behavior_base_dir mouse '\'];
    cd(d)
%     load('behavior.mat') % loading b of the mouse (all the sessions)
    load(['behavior_',mouse,'.mat']) % loading b of the mouse (all the sessions)
    session_names = sessions{mi};
    hfm = []; % hit fa miss
...
%                 session_lengths{mi}(i) = length(b{i}.hitTrialInds);
                session_lengths_nomiss{mi}(i) = length(b{i}.hitTrialNums) + length(b{i}.faTrialNums);
            end
        end
    end
    hf{mi} = hfm; % hit & fa
    ...
    hf{mi} = abs(hf{mi}-2); % 1 for hit, 0 for miss    
    correct_rate_100{mi} = zeros(length(hf{mi}),1);
    for i = 100 : length(hf{mi})
        correct_rate_100{mi}(i) = sum(hf{mi}(i-99:i));
    end
    LearningEM(hf{mi},1,0.5);
end
...
%         temphc = smooth(histcounts(union(cell2mat(tempwl), cell2mat(tempcl)), bins)) / step / length(tempwl);
        temphc = smooth(histcounts(cell2mat(tempwl), bins)) / step / length(tempwl);
...
%         temphc = smooth(histcounts(union(cell2mat(tempwl), cell2mat(tempcl)), bins)) / step / length(tempwl);
        temphc = smooth(histcounts(cell2mat(tempcl), bins)) / step / length(tempwl);
