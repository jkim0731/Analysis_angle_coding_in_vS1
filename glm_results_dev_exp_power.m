...
for ci = 1 : numCells    
    ...
    % just for now, compare between subtracting method and gain method
    % subtracting method first
    ...
    % now gain method
    tempPartialDEgain = zeros(1,length(indPartial));
    for pi = 1 : length(indPartial)
        partialInds = [1,indPartial{pi}+1]; % including intercept
        partialCoeffs = coeff(partialInds);
        partialModel = exp([ones(length(finiteIndTest),1),testInput(finiteIndTest,indPartial{pi})]*partialCoeffs);
        partialLogLikelihood = sum(log(poisspdf(spkTest',partialModel)));
        partialDevExp = (partialLogLikelihood - nullLogLikelihood)/(saturatedLogLikelihood - nullLogLikelihood);
        tempPartialDEgain(pi) = partialDevExp;
    end
    
    % assigning values within parfor loop
    allDE(ci) = devExplained;
    partialDEsub{ci} = tempPartialDEsub;
    partialDEgain{ci} = tempPartialDEgain;
    calcium{ci} = dF;
    spikes{ci} = spkTest;
    fullModel{ci} = model;
...
devExp.partialGain = partialDEgain;
