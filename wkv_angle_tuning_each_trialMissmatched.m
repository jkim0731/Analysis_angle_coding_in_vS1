...
%% Bottom is obsolete. Change allPredictors directly.
% original files are copied and saved as *_OG.mat
baseDir = 'Y:\Whiskernas\JK\suite2p\038\';
mouse = 38;
session = 2;
...
% for ri = 1 : 10      
%     glmfn = sprintf('glmResponseType_JK%03dS%02d_lasso_NC_R%02d',mouse, session, ri); % these are only from all active cells
    glmfn = sprintf('glmResponseType_JK%03dS%02d_lasso_NC_R10',mouse, session); % these are only from all active cells
    load([baseDir, glmfn], 'allPredictors')
    ...
    naninds = find(isnan(allPredictors{5}(:,1)));
    nanindChanges = find(diff(naninds)>1);
    numTrials = length(nanindChanges);
    ap2temp = allPredictors;
    % confirmed that this numTrials is 1 larger than
    % lowerplaneTrialNums
    for i = 5 : 8
        allPredictors{i}(naninds(nanindChanges(ind2removeFromAp2))-4+1 : naninds(nanindChanges(ind2removeFromAp2+1))-4,:) = [];
    end

    save([baseDir, glmfn], 'allPredictors', '-append')
% end
