...
cd(sprintf('%s%03d','Y:\Whiskernas\JK\suite2p\', mice(mi)))
ac = load(sprintf('JK%03dS%02dangle_tuning_all_cell.mat',mice(mi),sessions{mi}(1)));
glm = load(sprintf('JK%03dS%02dangle_tuning.mat',mice(mi),sessions{mi}(1)));
cd('Y:\Whiskernas\JK\suite2p\')
load('glm_results_responseType.mat', 'naive')
...
indNTRanovaSetdiffGlm = ismember(ac.spk.touchID,idNTRanovaSetdiffGlm);
indNTRGlm = ismember(ac.spk.touchID,idNTRglm);
