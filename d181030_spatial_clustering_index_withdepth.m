...
for mi = 1 : length(mice)
    ...
    for si = 1 : length(sessions{mi})
        ...
        savefn = sprintf('JK%03dS%02dsingleCell_spatial_clustering_index_withdepth_c2only.mat', mouse, session);
            ...
            for i = 1 : length(angles)
                ...
                if length(currAngleInd) > 1
                    ...
                    for j = 1 : length(withinGroupDist{i})
                        ...
                        withinGroupDist{i}(j) = sqrt((((celly(ind1) - celly(ind2))^2 + (cellx(ind1) - cellx(ind2))^2) * u.pixResolution)^2 + (cellDepths(ind1) - cellDepths(ind2))^2);
                    end
                else
                    withinGroupDist{i} = [];
                end
                    ...
                    if isempty(currOtherInd)
                        ...
                        for j = 1 : length(currAngleInd)
                            ...
                            for k = 1 : length(currOtherInd)            
                                ...
                                betweenGroupDist{i}((j-1)*length(currOtherInd) + k) = sqrt((((celly(ind1) - celly(ind2))^2 + (cellx(ind1) - cellx(ind2))^2) * u.pixResolution)^2 + (cellDepths(ind1) - cellDepths(ind2))^2);
                            end
                        end
                    end
                end                    
                ...
                for i = 1 : length(angles)
                    ...
                    if length(currAngleInd) > 1
                        ...
                        for j = 1 : length(pwithinGroupDist{i})
                            ind1 = tuneInds(currAngleComb(j,1));
                            ind2 = tuneInds(currAngleComb(j,2));
                            pwithinGroupDist{i}(j) = sqrt((((celly(ind1) - celly(ind2))^2 + (cellx(ind1) - cellx(ind2))^2) * u.pixResolution)^2 + (cellDepths(ind1) - cellDepths(ind2))^2);
                        end
                    else
                        pwithinGroupDist{i} = [];
                    end
                    ...
                    if isempty(currAngleInd)
                        ...
                        if isempty(currOtherInd)
                            ...
                            for j = 1 : length(currAngleInd)
                                ...
                                for k = 1 : length(currOtherInd)            
                                    ind2 = tuneInds(currOtherInd(k));
                                    pbetweenGroupDist{i}((j-1)*length(currOtherInd) + k) = sqrt((((celly(ind1) - celly(ind2))^2 + (cellx(ind1) - cellx(ind2))^2) * u.pixResolution)^2 + (cellDepths(ind1) - cellDepths(ind2))^2);
                                end
                            end
                        end
                    end
