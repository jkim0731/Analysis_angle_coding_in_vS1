...
%     for si = 2
        ...
        savefn = sprintf('angle_tuning_model_touchCell_NC_preLick_perTouch_JK%03dS%02d', mouse, session);
...
%         (4-44) explanations later
        ...
        for pi = 1 : numPlane
            ...
            for ti = 1 : length(planeTrialsInd{pi})
                ...
                preLickInd = find(cellfun(@(x) tempTrial.whiskerTime(x(1)) < tempLickTime, tempTrial.protractionTouchChunksByWhisking));
...
%                             thetaAtTouchMat, phiAtTouchMat, kappaHAtTouchMat, kappaVAtTouchMat, arcLengthAtTouchMat, touchCountMat];
            ...
            % #4~#15: from full wkv glm (#3), remove each one
            % #16: remove all whisker variables
